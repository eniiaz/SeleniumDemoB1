...
public class Blazer {
    ...
    public static void flightNumberUniquenessVerification(){
        System.out.println("\nVerifying flight number uniqueness scenario");
        driver.get("https://www.blazedemo.com/reserve.php");
        List<WebElement> flights = getColumnCells(2);

        Set<String> uniqueFlight = new HashSet<>();
        for (WebElement flight: flights){
            uniqueFlight.add(flight.getText().trim());
        }
        // check the size of Set and the original List of WebElement
        // If they are equal that means all flgiht numbers are unique
        if (uniqueFlight.size() == flights.size()){
